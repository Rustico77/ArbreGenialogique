...
import { createPerson, deletePerson, getAllPerson, getAllRelation, getSinglePerson, updatePersonPosition, updateRelation } from "@/app/actions/personActions";
...
export default function FamilyTree() {
  ...
  const onConnect = useCallback(
    (connection) => {
      ...
      const newEdge = { ...connection, id: `${connection.source}-${connection.target}`, label: RELATIONTYPE[0] };
  ...
  const handleSaveModif = async () => {
    ...
    nodes.forEach(async (node) => {
      const res = await updatePersonPosition(node.id, node.position.x, node.position.y);
      if(!res.isSuccess){
        isDone = false;
      }
    });
    ...
    // Relation update
    const res = await updateRelation(projectSelected.id, edges.map(edge => ({
  ...
  const GetAllPerson = async (projectId) => {
      setNodes([]);
      const res = await getAllPerson(projectId);
      if(res.data.length > 0){
  ...
  return (
    <div style={{ width: "100%", height: "85vh" }}>
      {/* Modal */}
      <CreatePersonModal open={openModal} setOpen={setOpenModal} projectId={projectSelected?.id} personCount={nodes.length} person={currentPerson} action={() => GetAllPerson(projectSelected?.id)} />

      {/* header */}
      <div className="h-fit py-5 px-10">
        {projects.length === 0 ?(
          <p className="flex justify-center text-white text-center">
            Pas de projet disponible ! Veuillez en créer pour définir son arbre
          </p>
        ) : 
        <div className="flex space-x-10 overflow-x-auto py-2">
          ...
          <Button
            ...
            className={`p-7 shadow-md rounded-xl border hover:shadow-lg hover:text-white transition cursor-pointer
              ${project.id === projectSelected.id ? "bg-primary text-white" : "bg-white text-black"}`
            }
          >
      ...
      {projects.length > 0 ?
        (
          <div className="flex justify-center space-x-5 mx-2">
            {/* Ajouter une personne */}
            <Button 
              onClick={() => {setOpenModal(true), setCurrentPerson(null)}}
              className="bg-red-800 p-5 cursor-pointer hover:bg-red-700"
            >
                + Ajouter une personne
            </Button>
            ...
            <Button 
              ...
              className="bg-green-800 p-5 cursor-pointer hover:bg-green-700"
            >
                Enrégistrer les modifs
            </Button>
